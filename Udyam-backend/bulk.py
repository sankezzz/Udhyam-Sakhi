...
def send_bill_to_seller(bill_data):
    seller_number = "919730182225"  # 👈 Replace this with actual seller number
...
def send_bulk_orders_to_seller(bulk_orders_data):
    seller_number = "919730182225"

    message = f"📦 *New Bulk Orders Received!*\n\n"

    for idx, order in enumerate(bulk_orders_data, 1):
        user_id = order.get('username') or order.get('user_id')
        order['user_id'] = user_id  # ensure uniform key
    ...
    response = requests.post(url, headers=headers, json=payload)
    print(f"📤 Sent Bulk Orders to Seller Response: {response.json()}")
...
def generate_bulk_bill(user_id, orders):
    ...
    # ✅ Constant user_id override
    user_id = "919730182225"
    ...
    bill_message += f"\n💰 *Total bill is ₹{total_cost}*"
    bill_message += f"\n💰 *Total Payable in Advance is 10% of the total value: ₹{advanced}*"
    send_message(user_id, bill_message)
    ...
    try:
        ...
        send_message(user_id, "✅ Please complete the payment . This is an advanced payment that you have made for your order further payment will be completed on the delivery . You'll get an auto-confirmation once done.")
        send_message("Payment Successfull")
...
def get_latest_order():
    latest_file = "orders.json"  # or "orders/order.json"
...
def add_bulk_orders():
    ...
    try:
        ...
        for order in orders:
            order['user_id'] = order.pop('username')

        # 📝 Save orders to JSONL file (newline-delimited)
        with open("orders.json", "a") as f:
            for order in orders:
                json.dump(order, f)
                f.write("\n")
        ...
        # 📤 Optional: Send all orders to seller
        send_bulk_orders_to_seller(orders)

        # 💳 Generate one bill and one Razorpay link per user
